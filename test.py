...
def tukey_hsd_test(expert_aucs, moe_aucs):
    data = []
    for i, aucs in expert_aucs.items():
        data.extend([(auc, f"Expert_{i}") for auc in aucs])
    data.extend([(auc, "MoE") for auc in moe_aucs])
    df = pd.DataFrame(data, columns=["AUC", "Model"])
    tukey_result = pairwise_tukeyhsd(df["AUC"], df["Model"], alpha=0.05)
    return tukey_result
...
def main():
    ...
    for idx, ood_loader in enumerate(ood_loaders):
        ...
        tukey_result = tukey_hsd_test(expert_aucs, moe_aucs)
