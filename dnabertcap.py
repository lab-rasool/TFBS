...
def squash(vectors, axis=-1):
    ...
    scale = (
        s_squared_norm / (1 + s_squared_norm) / tf.sqrt(s_squared_norm + K.epsilon())
    )
...
class CapsuleLayer(layers.Layer):
    ...
    def call(self, inputs, training=None):
        ...
        b = tf.zeros(
            shape=[tf.shape(inputs)[0], self.num_capsule, 1, self.input_num_capsule]
        )
        ...
        for i in range(self.num_routing):
            ...
            c = tf.transpose(
                tf.nn.softmax(tf.transpose(b, perm=[0, 2, 3, 1])), perm=[0, 3, 1, 2]
            )
            print("c is:", c)
            ...
            if i < self.num_routing - 1:
                ...
                b += tf.matmul(
                    outputs, inputs_hat, transpose_b=True
...
def CapsNet(input_shape, n_class, num_routing):
    ...
    lstm = Bidirectional(
        LSTM(64, dropout=dropout_p, recurrent_dropout=dropout_p, return_sequences=True)
    )(conv1)

    # Layer 2: Conv1D layer with `squash` activation, then reshape to [None, num_capsule, dim_vector]
    primarycaps = PrimaryCap(
        lstm, dim_vector=8, n_channels=8, kernel_size=2, strides=2, padding="valid"
    )

    # print("PrimaryCaps:",primarycaps.shape)
    # Layer 3: Capsule layer. Routing algorithm works here.
    digitcaps = CapsuleLayer(
        num_capsule=n_class, dim_vector=16, num_routing=num_routing, name="digitcaps"
    )(primarycaps)
...
model = CapsNet(input_shape=(98, 768), n_class=2, num_routing=3)
model.summary()

model.compile(
    optimizer="adam", loss=margin_loss, metrics=[tf.keras.metrics.BinaryAccuracy()]
)
...
for jj in range(0, 20):
    X_train, X_test, y_train, y_test = train_test_split(
        features, dummy_y, test_size=0.20, random_state=0
    )
    ...
    model.fit(
        X_train, y_train, batch_size=64, epochs=12, validation_split=0.10, verbose=1
    )
    ...
    print(
        "Specificity is:",
        specificity_score(test_true, test_prediction, average="weighted"),
    )
